...
export default function ColSize({
  ...
  setIsDelete,
  setSelectedId,
  handleUpdateSetSubCategories,
}) {
  const [isEdit, setIsEdit] = useState(false);
  const [newSize, setNewSize] = useState(subCategory.name);
  const handleUpdateSize = async () => {
    try {
      const res = await updateSize({ idSize: subCategory.id, name: newSize });
      notifySuccess(res.message);
      handleUpdateSetSubCategories(res.result);
      setIsEdit(false);
    } catch (err) {
      console.log(err);
  ...
  return (
    <tr className="border-b border-gray-200 hover:bg-gray-100">
      <td className="py-2 px-6 text-left">
        <div className="font-medium">{subCategory.id}</div>
      </td>
        ...
        {isEdit && (
          <div className="flex gap-2">
            <button
              ...
              disabled={newSize === subCategory.name}
